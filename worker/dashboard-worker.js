/**
 * easy MV 실시간 서버 (Cloudflare Worker) — 대시보드 붙여넣기용 (순수 JavaScript)
 *
 * 이 파일은 worker/src/index.ts 와 완전히 동일한 로직을 TypeScript 없이 옮긴 버전입니다.
 * Cloudflare 대시보드의 코드 편집기(Quick Edit)는 TypeScript 빌드를 지원하지 않으므로,
 * 터미널(wrangler) 없이 대시보드에서만 배포하려면 이 파일 내용을 그대로 붙여넣으세요.
 * 배포 절차는 worker/README.md의 "웹 대시보드로만 배포하기" 항목을 참고하세요.
 *
 * 저장소:
...
const drawKey = (id, index) => `draw/${id}/${index}.jpg`

async function readProject(env, id) {
  return env.PROJECTS.get(key(id), 'json')
}
async function writeProject(env, p) {
...
function authorized(request, env, p) {
...
function normalizeIncoming(body, existing) {
  const pages = body.pages.map((pg, i) => {
